...
assert pkgs.lib.hasInfix "background-color: #000000;" regreet.extraCss;
...
pkgs.runCommandLocal "hyprland-nvidia-check"
    ...
    ''
        ...
        grep -Fq ':y "0px"' "$ewwYuck"
        grep -Fq ':height "38px"' "$ewwYuck"
        grep -Eq '/nix/store/[^/]+-coreutils-[^/]+/bin/date "\+%a %b %d %H:%M"' "$ewwYuck"
        grep -Fq ':class "clock"' "$ewwYuck"
        # The greeter bar mirrors the desktop bar: true black canvas, the same
        # lightened pill surface, and the same stepped clock size.
        grep -Fq 'background-color: #000000;' "$ewwScss"
        grep -Fq 'background-color: #292e42;' "$ewwScss"
        grep -Fq 'font-size: 15px;' "$ewwScss"
        if grep -Fq ':class "island clock"' "$ewwYuck"; then
            echo 'Greeter date-time must not be an island' >&2
            exit 1
        fi
        grep -Eq '/nix/store/[^/]+-fastfetch-[^/]+/bin/fastfetch --logo none --pipe' "$ewwYuck"
        grep -Fq 'font-size: 13px;' "$ewwScss"
        grep -Eq 'background-color: #[0-9a-f]{6};' "$ewwScss"
        grep -Fq 'border: 0;' "$ewwScss"
