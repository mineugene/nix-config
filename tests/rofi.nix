...
pkgs.runCommandLocal "rofi-command-check"
    ...
    ''
        ...
        # rofi sizes its rows and icons from the font, so the launcher's own
        # size is what makes a long list fit on one screen.
        require_line "$darkPalette" '    font-interface-small: "IosevkaNF 11";'
        require_line "$darkPalette" '    radius-card: 6px;'
        require_line "$darkPalette" '    radius-pill: 999px;'
        require_line "$darkPalette" '    spacing-normal: 16px;'
        # rofi cannot compose a padding pair from two variables, so the pairs
        # exist as their own tokens rather than at each use.
        require_line "$darkPalette" '    padding-row: 8px 16px;'
        require_line "$darkPalette" '    padding-window: 16px 24px;'
        ...
        # A width in characters is a measure of the text the list holds, so
        # one value serves every monitor and no breakpoint can go stale.
        require_line "$baseTheme" '    width: 56ch;'
        require_line "$baseTheme" '    font: @font-interface-small;'
        # Rows carry their own padding, so a gap between them as well reads as
        # separate cards rather than one list.
        grep -A3 -F 'listview {' "$baseTheme" | grep -Fq 'spacing: 0;' || {
            echo 'List rows must not be separated by a gap as well as padding' >&2
            exit 1
        }
        # An icon glyph paints past the advance rofi measures it by.
        require_line "$baseTheme" '    padding: 0 0.5em 0 0;'
        if grep -Fq '@media' "$baseTheme"; then
            echo 'A width measured in text needs no per-monitor breakpoint' >&2
            exit 1
        fi
        # The window's inset matches the terminal's, its rows are padded as
        # the bar's dropdown rows are, and both take the window's corners.
        require_line "$baseTheme" '    padding: @padding-window;'
        require_line "$baseTheme" '    padding: @padding-row;'
        require_line "$baseTheme" '    border-radius: @radius-card;'
        if grep -Fq 'radius-pill' "$baseTheme"; then
            echo 'A field inside a window follows the window corners' >&2
            exit 1
        fi
        # The launcher must highlight a row the way the bar's dropdown does.
        grep -A3 -F 'element selected {' "$baseTheme" \
            | grep -Fq 'background-color: @surface-hover;' || {
            echo 'Selected rows must use the shared hover background' >&2
            exit 1
        }
        grep -A3 -F 'element selected {' "$baseTheme" \
            | grep -Fq 'text-color: @foreground;' || {
            echo 'Selected rows must keep the shared foreground' >&2
            exit 1
        }
        ...
        # The prompt is the mode name unless it is told otherwise, and "drun"
        # names an implementation rather than what the field does.
        grep -Fq 'display-drun: "";' "$launcherTheme" || {
            echo 'The launcher prompt must be the search glyph' >&2
            exit 1
        }
        ...
        require_line "$powerTheme" '    lines: 6;'
