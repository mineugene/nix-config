...
pkgs.runCommandLocal "hyprland-lua-check"
    ...
    ''
        ...
        require_line "$monitors" '  for _, rule in ipairs(monitor) do'
        require_line "$monitors" '    hl.monitor(rule)'
        ...
        require_line "$configuredMonitor" '    output = "DP-2",'
        require_line "$configuredMonitor" '    mode = "1920x1080@60.0",'
        require_line "$configuredMonitor" '    bitdepth = 10,'
        require_line "$configuredMonitor" '    cm = "hdr",'
        require_line "$configuredMonitor" '    sdrsaturation = 1.05,'
        require_line "$configuredMonitor" '    sdr_min_luminance = 0.005,'
        require_line "$configuredMonitor" '    sdr_max_luminance = 250,'

        # SDR-in-HDR knobs emit only when set: the default `sdrbrightness`
        # stays out of the generated rule entirely.
        reject_text "$configuredMonitor" 'sdrbrightness'
        test "$(grep -Fxc '    sdrsaturation = 1.05,' "$configuredMonitor")" -eq 1
        test "$(grep -Fxc '    sdr_min_luminance = 0.005,' "$configuredMonitor")" -eq 1
        test "$(grep -Fxc '    sdr_max_luminance = 250,' "$configuredMonitor")" -eq 1
        ...
        # The lock background follows the palette, so assert its shape only.
        grep -Eq '^  color=rgb\([0-9a-f]{6}\)$' "$hyprlockFile"
        ...
        # No Hyprland branding or wallpaper is rendered beneath clients. Keep
        # the fallback clear true black to avoid unnecessary OLED emission.
        ...
        require_line "$root" '    ["key_press_enables_dpms"] = true,'
        require_line "$root" '    ["mouse_move_enables_dpms"] = true'
        ...
        for program in terminal browser lock ui_launcher ui_clipboard ui_power eww_audio monitor_topology wl_copy grim slurp wpctl; do
