...
# Run all pre-commit checks, using the active or cached direnv shell when available.
[group('maintenance')]
lint:
	if [ -n "${DIRENV_DIR:-}" ]; then pre-commit run --all-files; elif command -v direnv >/dev/null; then direnv exec . pre-commit run --all-files; else nix develop --command pre-commit run --all-files; fi

# Format tracked files, then run all pre-commit checks.
[group('maintenance')]
fmt:
	if [ -n "${DIRENV_DIR:-}" ]; then just _fmt; elif command -v direnv >/dev/null; then direnv exec . just _fmt; else nix develop --command just _fmt; fi
