{ lib, options, ... }:
let
    # Home Manager 26.05+ declares per-widget settings in nested attrsets
    # (programs.fzf.fileWidget.{command,options}, etc.). Home Manager 25.11
    # declares flat *Widget* names and rejects the nested names. Detect the
    # available schema and emit only the options it declares.
