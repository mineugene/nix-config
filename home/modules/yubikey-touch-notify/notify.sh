#!/usr/bin/env bash
# Bash is required for arrays, process substitution, and fixed-width read -N.
...
flash_window_current_on() {
    ...
    "$tmux_command" set-option -g @yk-touching 1 2>/dev/null || true
    WINDOW_FLASH_ACTIVE=1
}
flash_window_current_off() {
    command -v "$tmux_command" >/dev/null 2>&1 && "$tmux_command" ls >/dev/null 2>&1 || return 0
    (( WINDOW_FLASH_ACTIVE )) || return 0
    "$tmux_command" set-option -gu @yk-touching 2>/dev/null || true
    WINDOW_FLASH_ACTIVE=0
}
...
ring_terminal_bells() {
    ...
    done < <("$tmux_command" list-clients -F '#{client_tty}' 2>/dev/null)
}
...
while :; do
    ...
    done < <("$socat_command" -u "UNIX-CONNECT:$SOCKET" STDOUT 2>/dev/null)
    sleep 1
done
