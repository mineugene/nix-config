...
focus_primary() {
    ...
    command=$(jq -r --argjson connected "$monitors" '
        ...
        def rotated:
            .transform == 1 or .transform == 3 or .transform == 5 or .transform == 7;
        def cursor_x:
            .x + ((if rotated then .height else .width end) / .scale / 2);
        def cursor_y:
            .y + ((if rotated then .width else .height end) / .scale / 2);
        ...
        | "hl.dispatch(hl.dsp.focus({ monitor = "
          + ($primary_monitor.name | tojson)
          + " })); hl.dispatch(hl.dsp.cursor.move({ x = "
          + ($primary_monitor | cursor_x | tojson)
          + ", y = "
          + ($primary_monitor | cursor_y | tojson)
          + " }))"
    ' "$layouts_file")
...
case ${1-} in
    ...
    watch)
        ...
        ;;
    *)
        printf 'usage: monitor-topology apply | save | watch\n' >&2
        exit 64
        ;;
esac
