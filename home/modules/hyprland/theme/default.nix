...
in
{
        ...
        fonts = {
            ...
            monospace = {
                ...
                package = lib.mkPackageOption pkgs "iosevka-term-nf" { } // {
                    description = ''
                        Package providing the monospace family. Only the icon metrics
                        build reads it, so it must contain the family named above.
                    '';
                };
            };
        };
    ...
    config = {
        ...
        # spacing step, so every term has to stay even.
        assertions =
            lib.mapAttrsToList
                (name: value: {
                    assertion = lib.mod value 2 == 0;
                    message = "mine.desktop.theme.${name} must be even to keep pill end caps circular, got ${toString value}.";
                })
                {
